...
let req=""
function timeToMixJuice (req) {
    let juices = Object.keys(prepTime)
    let times = Object.values(prepTime)
    if(juices.includes(req)){
    for (let i = 0; i < juices.length; i++) {
       
        if (juices[i]===req) {
            console.log(`It will be ready in ${times[i]} minutes`)
            return times[i]
            }
        }
    }
    else {
        console.log(`It will be ready in ${prepTime['All other']} minutes`) 
        return prepTime['All other']
    }
}
timeToMixJuice(`Tropical Island`)
timeToMixJuice('Berries & Lime')
...
let stock = 0
let supply = []
function limesToCut(needsToCut,supply) {
    let sum = []
    for (let i = 0; i<supply.length; i++) {
        if (supply[i] in lemons) {
            sum.push(lemons[supply[i]])
            stock = sum.reduce((a, b) => a + b, 0)
            if (stock>needsToCut){break}
        }
    }
    if (needsToCut < stock) {
        console.log(`You need to cut ${sum.length} limes`);
    } else {
        console.log("You don't have enough limes");
    }
}
limesToCut(25, ['small', 'small', 'large', 'medium', 'small'])
...
// **Exercise 3**
let orders = []
function remainingOrders(timeToGoHome,orders) {
    let sum = []
    for (let i = 0; i<orders.length; i++) {
        if (orders[i] in prepTime) {
            sum.push(prepTime[orders[i]])
            timeToMake = sum.reduce((a, b) => a + b, 0)
            if (timeToMake>timeToGoHome) {orders.splice(0,i+1)}
        }
    }
    if (orders.length===0) {
        console.log(`Li Mei finished everything`);
    } else {
        console.log(`Drinks left to make: ${orders}`)
    }
}
remainingOrders(5, ['Energizer', 'All or Nothing', 'Green Garden'])
